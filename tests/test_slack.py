...
def test_get_slack_client(set_env):
...
def test_get_slack_client_no_token(monkeypatch):
    monkeypatch.delenv("SLACK_TOKEN", raising=False)
    client = get_slack_client()
    assert client is None


def test_get_slack_channel(set_env):
    set_env({"SLACK_CHANNEL": "test-channel"})
    channel = get_slack_channel()
    assert channel == "test-channel"


def test_get_slack_channel_no_channel(monkeypatch):
...
@pytest.fixture
def mock_slack_channel(mocker):
    return mocker.patch("eevee.slack.get_slack_channel")


def test_post_slack_message(mock_slack_client, mock_slack_channel, mocker):
...
def test_post_slack_message_no_client(mock_slack_client, mock_slack_channel):
...
def test_post_slack_message_no_channel(mock_slack_client, mock_slack_channel):
